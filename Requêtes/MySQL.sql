...
-- version MySQL :
-- ------------------------------------------ QUESTION 1 -------------------------------------------------- --
-- Requête filtrant des données à l'aide d'une expression rationnelle (REGEXP) sur un champ textuel joueur_prenom.
SELECT joueur_nom, joueur_prenom
FROM P16_Joueur
WHERE joueur_prenom REGEXP '(.)\\1' -- 1. Vérifie que le prénom contient un caractère doublé (ex : "aa", "ee").
  AND joueur_prenom REGEXP '[^aeiouAEIOU]$' -- 2. Vérifie que le prénom se termine par une consonne (ex : "b", "r", etc.).
  AND joueur_prenom REGEXP '(?<=.)n'; -- 3. Vérifie que le prénom contient un "n" qui est précédé de n'importe quel caractère.
...
SELECT M.match_date, M.match_lieu, C.club_nom, COALESCE(D.nb_buts, 0) AS buts
FROM P16_Match M
...
FROM P16_Club
WHERE club_id IN (SELECT club_id FROM P16_Participe WHERE edition_num IN (SELECT edition_num FROM P16_Edition WHERE edition_annee = 2024))
UNION
SELECT club_id, club_nom
FROM P16_Club
WHERE club_id IN (SELECT club_id FROM P16_Participe WHERE edition_num IN (SELECT edition_num FROM P16_Edition WHERE edition_annee = 2025));

-- INTERSECT
-- MySQL ne supporte pas directement l'opérateur INTERSECT(On fait semblant ^^). On peut simuler cela avec une jointure.
-- Sélectionne les clubs ayant participé aux éditions 2025 et 2024
SELECT DISTINCT c.club_id, c.club_nom
FROM P16_Club c
         JOIN P16_Participe p1 ON c.club_id = p1.club_id
         JOIN P16_Participe p2 ON c.club_id = p2.club_id
WHERE p1.edition_num IN (
    SELECT edition_num
    FROM P16_Edition
    WHERE edition_annee = 2024
)
  AND p2.edition_num IN (
    SELECT edition_num
    FROM P16_Edition
...
-- EXCEPT
-- MySQL ne supporte pas l'opérateur EXCEPT directement(On fait semblant aussi ^^). On peut le simuler simplement avec des conditions dans la clause where (NOT IN).
...
WHERE match_id = (
    ...
    WHERE club_id = (
        SELECT club_id
        FROM P16_Edition
        WHERE edition_annee = 2024) AND terrain = 'neutre'
...
WHERE joueur_id IN ( -- ON NE PEUT PAS FAIRE '=' car elle renvoit plusieurs lignes le IN est indispensable.
        SELECT joueur_id FROM P16_Compose
        WHERE club_id IN (
                SELECT club_id FROM P16_Club
...
FROM (SELECT nb_points FROM P16_Participe WHERE statut_qualif = 'Qualifié') AS pointsClub_qualifies;  -- Sous-requête des clubs qualifiés
...
WHERE joueur_id IN (
    SELECT joueur_id
    FROM P16_Compose
    WHERE club_id = (SELECT club_id FROM P16_Club WHERE club_nom = 'Real madrid')  -- Trouve l'id du club REAL MADRID
);
...
WHERE d.nb_buts = (
    SELECT SUM(nb_buts)
    FROM P16_Dispute d2
    WHERE d2.match_id = d.match_id AND d2.club_id != d.club_id
);
...
WHERE joue.buts_marques > ANY (
    ...
    WHERE comp2.club_id = comp.club_id AND joue2.match_id = joue.match_id AND joue2.joueur_id != j.joueur_id
);
...
WHERE j.joueur_id IN (
    ...
    WHERE pj.buts_marques > ALL (
        ...
        FROM P16_Joue pj2
        WHERE pj2.match_id = pj.match_id AND pj2.joueur_id != pj.joueur_id
    )
);
...
WHERE j.joueur_id IN (
    SELECT joueur_id
    FROM P16_Compose
    WHERE club_id = (SELECT club_id FROM P16_Club  WHERE club_nom = 'FC Barcelona')  -- Sous-requête pour trouver l'id du club FC Barcelone
);
...
FROM P16_Participe p NATURAL JOIN P16_Club
...
FROM
    P16_Dispute d1
        INNER JOIN
    P16_Dispute d2 ON d1.match_id = d2.match_id
        INNER JOIN
