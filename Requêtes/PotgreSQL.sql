...
-- version PostgreSQL :
-- ------------------------------------------ QUESTION 1 -------------------------------------------------- --
-- Requête filtrant des données à l'aide de (~) (PostgreSQL utilise ~ au lieu de REGEXP) sur un champ textuel joueur_prenom.
SELECT joueur_nom, joueur_prenom
FROM P16_Joueur
WHERE joueur_prenom ~* '(.)\1' -- Vérifie que le prénom contient un caractère doublé (ex : "aa", "ee"), le * pour prendre en compte les maj
  -- par exemple 'Aa'
  AND joueur_prenom ~ '[^aeiouAEIOU]$' -- Vérifie que le prénom se termine par une consonne.
  AND joueur_prenom ~ '(?<=.)n'; -- Vérifie que le prénom contient un "n" précédé d'un caractère.
...
-- LEFT JOIN :
-- execution: 11 ms
-- Plus lent que l'INNER JOIN; ici dans notre exemple ça ne se remarque pas trop car on a qu'une seule édition qui n'a pas de club_nom, mais quand meme il est un petiit peu lent car il doit retourner quand meme la ligne de la table gauche (P16_Edition), même si elle n'a pas de  correspondance dans la table droite (P16_Club), remplissant la colonne manquante par un NULL.
